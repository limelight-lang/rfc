...
| deferred increment | A `+1` owed to a COW child's count at the reset's reconciliation: an edge a survivor held at its promotion, recorded then because the count captured at that instant is discarded whole; a release the edge earns later, by the holder's teardown or by a store into its slot, is inside the child's delta (`ll-model/src/memory/reset_window.rs`, `Correction`) | deferred increment (Deutsch and Bobrow 1976); increment buffer (Bacon, Attanasio, Lee, Rajan and Smith 2001) | keep; replaces `ResetWindow::escrow` and `Corrections::escrowed` |
| deferred decrement | A `-1` owed at the same reconciliation: the compensating retain the counting pass gave an already promoted COW child, whose edge is in the log as well | deferred decrement (same sources) | keep; replaces `ResetWindow::credits`, `credit` and `Corrections::credited` |
...
| ownership mark | Header bit a store into a compiler-proven slot moves onto the slot's occupant; while it stands the release path registers no candidate for the entity and the holder's `dispose` destroys it instead of releasing it (`model/gc/strategies.md`, "The store barrier, as micro-operations") | owner bit; unique-ownership annotation | keep; the *proven slot* is the holder, and *owner* alone is ambiguous here |
...
| dead in place | A slot whose occupant has been freed and whose storage has not been handed out again: the mark stands in the slot's own header, set by the free before any path withholds or returns the slot, the count still reading zero, and a second free of the slot is refused on it; a slot on its block's free list and one whose return a trace withholds both carry it (`model/gc/rc-cycle.md`, "Zero-count entities pending slot reuse") | tombstone; logical deletion | keep; the mark is the third answer to whether the allocator may have a slot, beside live and free |
...
- *door* resolves to *allocation path* (a route a request for memory takes),
  ...
  *mailbox*, *channel* or *store-barrier form* (one of the barrier's
  ...
  named exactly. The crate's `ll-model/dev/design/door-sites.md`, deleted on
  2026-09-04 and kept by git, classified its sites against this list.
