...
the close of a collection over P, which reads no R, arms `Retire` again
while the count stands at its figure; nothing else arms (amended 2026-09-24
from two values: the retirement by a count; amended 2026-09-25 with the
ask, a value of the byte so that the mutator's side reads no collector cap,
and with the close over P's re-arming). The fire spends the
