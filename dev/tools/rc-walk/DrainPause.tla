...
(* between the last cell and `unguard`, and between `unguard` and the     *)
(* release of the external children. Neither invariant objects to them:   *)
(* the refusal rests on `unguard` running once, not on what a collection  *)
(* started there would find.                                              *)
(*                                                                        *)
(* Three abstractions, each stated because each bounds the result:        *)
(*                                                                        *)
