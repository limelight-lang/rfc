...
Toolchain: Java (tested on 19.0.1 and on 21.0.11) + `tla2tools.jar` (TLC2 2.19,
2024-08-08, from the official tlaplus GitHub releases; vendored here).

> **Protocol drift (2026-07-27):** `RcWalk.tla` and `DrainWindow.tla`
> model the pre-amendment protocol; the eager-death amendment
> (rc-walk.md) retired the condemned byte, the F5 death deferral and
> message-based acquittals. See the banner note in rc-walk-model.md;
> re-derivation is deferred to the checker-extensions work after design
> freeze. `DrainPause.tla` (2026-08-23) is written over the protocol in
> force: the mutator frees, and the sever of one entity may be split at
> cell granularity.
...
- `DrainPause.tla` + `DP_*.cfg` — what a synchronous collection may
  condemn while a drain is paused at a permitted boundary
  ([../../../model/gc/walk/questions.md](../../../model/gc/walk/questions.md),
  node D3): four configs, two that must exhaust clean and two kills. The
  population carries a second garbage cycle the collection is free to reap
  at every boundary, so a clean run says the paused component is pinned
  rather than that the model collects nothing.
