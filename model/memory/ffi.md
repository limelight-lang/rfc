...
> tier-3 auto-`FFIBox` rather than a compile error (accept-every-program); a
> foreign property is always a `FFIBox`, so there is no bare "owned-foreign"
> slot kind; freeing is the class's own `__destruct` (no separate hook);
> and the `FFIBox` records its wrapped type as an instance field. Still open,
> in [BACKLOG.md](../../BACKLOG.md): the `#[Borrow]`-into-`FFIBox` UAF and the
...
falls to the **tier-3 fallback: a managed `FFIBox` wraps it**, supplying the
`RcHeader` the headerless struct lacks, and the `FFIBox` becomes the owner.
...
1. **A managed (ARC) object** — a property anchors the C structure. A
   ...
   property holds a **`FFIBox`** wrapping it (a store into a class property is
   exactly an escape, "Escape" below). The structure's lifetime is then the
   `FFIBox`'s, released in the owner's `dispose` (drop phase) before the
...
`dispose`, the end of a tier-1/2 scope, or a wrapping `FFIBox`'s teardown. A
...
### Escape: attaching to the managed world with `FFIBox`
...
- `FFIBox` is a managed, refcounted, GC-visible entity, always a **separate
  wrapper**: the C structure stays truly headerless and the `FFIBox` holds a
...
- The `FFIBox` kind (4) is a single class-less singleton but wraps
  *different* FFI types, so the `FFIBox` records its **wrapped type as an
...
- A C structure that never escapes — provably confined to a tier-1 local —
  ...
  freed by the compiler's scheduled `dispose`, and `FFIBox` never
  materializes. The FFIBox is the tier-3 fallback for everything else.

`FFIBox` appears **only** where a C structure is attached to the managed
...
property holds a `FFIBox`, not a bare struct — there is no separate
"owned-foreign" property slot kind.

**Terminology note.** This raw-struct `FFIBox` is a different thing from
the ValueBox of [values.md](../values.md) — the 16-byte tagged cell for
`mixed`. Both were once called "Box"; the 2026-07-27 rename
([layouts.md](../layouts.md)) removed the collision.
...
by a `FFIBox` when it escapes. A write to a borrowed view separates it into
...
- **What roots a wrapper only the C side holds** — a hole, reported 2026-08-24
  ...
  (`rfc`'s `archive/pre-rc-cycle`, `model/gc/rc-walk.md`), which `ll_retain` at the boundary
  ...
  `rfc`'s `archive/pre-rc-cycle`, `model/gc/walk/questions.md` carries it, with an idea
...
- **Extension interfaces on `#[FFI]` classes**
  ...
  reached from a descriptor at conversion time; a `FFIBox` can carry it,
